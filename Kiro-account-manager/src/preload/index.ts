...
const api = {
  ...
  // 账号管理 - 检查账号状�?  checkAccountStatus: (account: unknown): Promise<unknown> => {
    return ipcRenderer.invoke('check-account-status', account)
  },

  // 后台批量刷新账号（在主进程执行，不阻�?UI�?  backgroundBatchRefresh: (accounts: Array<{
    id: string
    email: string
    idp?: string
  ...
  // 后台批量检查账号状态（不刷�?Token�?  backgroundBatchCheck: (accounts: Array<{
    id: string
    email: string
    credentials: {
      accessToken: string
  ...
  // 监听后台检查进�?  onBackgroundCheckProgress: (callback: (data: { completed: number; total: number; success: number; failed: number }) => void): (() => void) => {
    const handler = (_event: Electron.IpcRendererEvent, data: { completed: number; total: number; success: number; failed: number }): void => {
      callback(data)
    }
  ...
  // 监听后台检查结果（单个账号�?  onBackgroundCheckResult: (callback: (data: { id: string; success: boolean; data?: unknown; error?: string }) => void): (() => void) => {
    const handler = (_event: Electron.IpcRendererEvent, data: { id: string; success: boolean; data?: unknown; error?: string }): void => {
      callback(data)
    }
  ...
  // 切换账号�?Kiro CLI - 写入凭证�?SQLite 数据�?  switchAccountCli: (credentials: {
    accessToken: string
    refreshToken: string
    clientId?: string
    clientSecret?: string
  ...
  // 文件操作 - 导出到文�?  exportToFile: (data: string, filename: string): Promise<boolean> => {
    return ipcRenderer.invoke('export-to-file', data, filename)
  },

  // 文件操作 - 从文件导�?  importFromFile: (): Promise<string | null> => {
    return ipcRenderer.invoke('import-from-file')
  },

  // 验证凭证并获取账号信�?  verifyAccountCredentials: (credentials: {
    refreshToken: string
    clientId: string
    clientSecret: string
    region?: string
  ...
  // �?AWS SSO Token (x-amz-sso_authn) 导入账号
  ...
  // 轮询 Builder ID 授权状�?  pollBuilderIdAuth: (region?: string): Promise<{
    success: boolean
    completed?: boolean
    status?: string
    accessToken?: string
  ...
  // 轮询 IAM SSO 授权状�?  pollIamSsoAuth: (region?: string): Promise<{
    success: boolean
    completed?: boolean
    status?: string
    accessToken?: string
  ...
  // 获取当前机器�?  machineIdGetCurrent: (): Promise<{
    success: boolean
    machineId?: string
    error?: string
  ...
  // 生成随机机器�?  machineIdGenerateRandom: (): Promise<string> => {
  ...
  // 请求管理员权限重�?  machineIdRequestAdminRestart: (): Promise<boolean> => {
  ...
  // 安装更新并重�?  installUpdate: (): Promise<void> => {
  ...
  // 获取反代池账号列�?  proxyGetAccounts: (): Promise<{ accounts: unknown[]; availableCount: number }> => {
    return ipcRenderer.invoke('proxy-get-accounts')
  },

  // 重置反代池状�?  proxyResetPool: (): Promise<{ success: boolean; error?: string }> => {
  ...
  // 监听反代状态变化事�?  onProxyStatusChange: (callback: (status: { running: boolean; port: number }) => void): (() => void) => {
  ...
  // 监听反代账号被封禁事件（TEMPORARILY_SUSPENDED / AccountSuspendedException�?  onProxyAccountSuspended: (callback: (info: { id: string; email?: string; reason: string; message: string; suspendedAt: number }) => void): (() => void) => {
  ...
  // 获取 K-Proxy 状�?  kproxyGetStatus: (): Promise<{ running: boolean; config: unknown; stats: unknown; caInfo: unknown }> => {
  ...
  // 检�?CA 证书是否已安�?  kproxyCheckCaCertInstalled: (): Promise<{ success: boolean; installed: boolean; error?: string }> => {
  ...
  // 安装 CA 证书到系统信任存�?  kproxyInstallCaCert: (): Promise<{ success: boolean; message?: string; error?: string }> => {
    return ipcRenderer.invoke('kproxy-install-ca-cert')
  },

  // 卸载 CA 证书从系统信任存�?  kproxyUninstallCaCert: (): Promise<{ success: boolean; message?: string; error?: string }> => {
  ...
  // 监听 K-Proxy 状态变化事�?  onKproxyStatusChange: (callback: (status: { running: boolean; port: number }) => void): (() => void) => {
    const handler = (_event: Electron.IpcRendererEvent, status: { running: boolean; port: number }): void => {
      callback(status)
    }
  ...
  // 监听显示关闭确认对话框事�?  onShowCloseConfirmDialog: (callback: () => void): (() => void) => {
    const handler = (): void => {
      callback()
    }
  ...
  // 获取注册状�?  registrationStatus: (): Promise<{ inProgress: boolean }> => {
