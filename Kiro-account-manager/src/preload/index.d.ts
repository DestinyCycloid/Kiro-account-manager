...
interface StatusResult {
  ...
  data?: {
    ...
    email?: string
    userId?: string
    idp?: string // 身份提供商：BuilderId, Google, Github �?    userStatus?: string // 用户状态：Active �?    featureFlags?: string[] // 特性开�?    subscriptionTitle?: string
    usage?: { 
      current: number
      limit: number
      percentUsed: number
    ...
    // 如果 token 被刷新，返回新凭�?    newCredentials?: {
      accessToken: string
      refreshToken?: string
      expiresAt?: number
    }
...
interface KiroApi {
  ...
  // 后台批量刷新（主进程执行，不阻塞 UI�?  backgroundBatchRefresh: (accounts: Array<{
    id: string
    email: string
    idp?: string
  ...
  // 获取反代服务器状�?  proxyGetStatus: () => Promise<{ running: boolean; config: unknown; stats: unknown; sessionStats?: { totalRequests: number; successRequests: number; failedRequests: number; startTime: number } }>
